# Mixture-of-Experts building blocks for the MoE-CTS method.
# Adapted from go2_rl_gym (wty-yy, https://github.com/wty-yy/go2_rl_gym),
# accompanying "Toward Reliable Sim-to-Real Predictability for MoE-based
# Robust Quadrupedal Locomotion" (RSS 2026). Pure PyTorch, simulator-agnostic.
...
class StudentMoEEncoder(nn.Module):
    ...
    ``forward`` returns only the latent, so the module is drop-in compatible
    with the host ``ActorCriticCTS.history_encoder`` call sites and stays
    TorchScript-exportable (PolicyExporterMoECTS scripts exactly this path).
    ``forward_with_weights`` additionally returns the gating weights, still
    attached to the autograd graph -- the only supported way to build the
    load-balance loss.

    There is deliberately no "weights of the last forward" attribute: the RL
    passes run this encoder under ``no_grad``, so whether such a snapshot
    carried gradients would depend on call order, and a load-balance loss built
    from it could silently become a constant.
    """
